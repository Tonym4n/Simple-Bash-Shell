...
void noPsHandler(int signum)                                //display interrupt msg and continue for no ps
{
    if(signum == SIGINT)
    {
...
struct cmd* parseredirs(struct cmd *cmd, char **ps, char *es)
{
    ...
    while(peek(ps, es, "<>")){
        ...
        switch(tok){
            ...
            case '+':  // >>
                cmd = init_redircmd(cmd, q, eq, O_WRONLY|O_CREAT, 3);                          //**********modified fd_to_close to 3
                break;
        }
    }
